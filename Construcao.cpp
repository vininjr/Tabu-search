...
void constroi_solucao_aleatoria(int n, int *s, float **d)
{
    ...
    while (j < n){
      ...
      int posicao_escolhida = random(0, tamanho_LC);
...
void constroi_solucao_parcialmente_gulosa_vizinho_mais_proximo(int n, int *s, float **d, float alpha)
{
    ...
    while (j < n){
      ...
      int posicao_escolhida = random(0, tamanho_LRC);
...
/* Constroi uma solucao pela inser��o mais barata */
...
//      mostra_lista(&inicio_LC, &final_LC);
    ...
    /* Insere as duas proximas cidades pela heur�stica do vizinho mais pr�ximo */
    while (j < 3){
      dist = INT_MAX;
      registro = inicio_LC;
      int pos = 1;
